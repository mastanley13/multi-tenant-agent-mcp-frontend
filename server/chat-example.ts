import { Agent, AgentInputItem, run, MCPServerStdio, withTrace, getAllMcpTools, user } from '@openai/agents';
...
async function initializeGHLAgent() {
  ...
  const mcpTools = await withTrace('load-ghl-mcp-tools', async () => {
...
async function startChat() {
  ...
  await withTrace('GHL AI Chat Session', async () => {
    while (true) {
      try {
        ...
        const result = await run(ghlAgent, history);
