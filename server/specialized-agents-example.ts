 ...
 */
abstract class BaseGHLAgent {
  protected agent: Agent;
  protected mcpServer: MCPServerStdio;
  ...
  async execute(input: string) {
    return await run(this.agent, input);
  }
...
class CRMAgent extends BaseGHLAgent {
  constructor(mcpServer: MCPServerStdio) {
    super(
      'GHL CRM Specialist',
      `You are a GoHighLevel CRM expert specializing in:
       - Contact management and segmentation
       - Task and note management  
       - Customer relationship optimization
       - Data organization and cleanup
       
       Focus on contact-related operations and relationship building activities.`,
      mcpServer
    );
  }
...
class SalesAgent extends BaseGHLAgent {
  constructor(mcpServer: MCPServerStdio) {
    super(
      'GHL Sales Specialist',
      `You are a GoHighLevel sales expert specializing in:
       - Opportunity creation and management
       - Pipeline optimization and tracking
       - Deal progression and forecasting
       - Sales workflow automation
       
       Focus on revenue-generating activities and sales process optimization.`,
      mcpServer
    );
  }
...
class MarketingAgent extends BaseGHLAgent {
  constructor(mcpServer: MCPServerStdio) {
    super(
      'GHL Marketing Specialist',
      `You are a GoHighLevel marketing expert specializing in:
       - Campaign creation and management
       - Automation workflow setup
       - Lead nurturing sequences
       - Marketing analytics and optimization
       
       Focus on marketing activities that drive engagement and conversions.`,
      mcpServer
    );
  }
...
class CustomerServiceAgent extends BaseGHLAgent {
  constructor(mcpServer: MCPServerStdio) {
    super(
      'GHL Customer Service Specialist',
      `You are a GoHighLevel customer service expert specializing in:
       - Customer support ticket management
       - Issue resolution and tracking
       - Customer satisfaction monitoring
       - Retention and follow-up activities
       
       Focus on maintaining excellent customer relationships and resolving issues efficiently.`,
      mcpServer
    );
  }
...
class WorkflowOrchestrator {
  ...
  async executeWorkflow(workflowDescription: string) {
    return await run(this.orchestratorAgent, workflowDescription);
  }
}
...
async function main() {
  ...
  try {
    await withTrace('GHL Specialized Agents Example', async () => {
      ...
      let result = await crmAgent.execute(
        'Create a new contact for Maria Garcia with email maria@startup.com and phone +1555123456. Add a note that she\'s interested in our premium package.'
      );
      ...
      result = await salesAgent.execute(
        'Create a new opportunity called "Enterprise Software License" worth $15,000 for Maria Garcia. Set it to the "Proposal" stage.'
      );
      console.log(result.finalOutput ?? result.output ?? result);
      console.log('');
